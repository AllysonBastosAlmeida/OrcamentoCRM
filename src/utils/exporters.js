...
export const exportQuoteToPDF = async (quote, options = {}) => {
  ...
  const sectionBoxGap = 4;
  const sectionGapBefore = 8;
  const sectionPadX = 5;
  const sectionPadTop = 8;
  const sectionPadBottom = 6;
  const sectionLineHeight = 4.5;
  const sectionFontSize = 8.7;
  const sectionTextWidth = contentWidth - sectionPadX * 2;

  const scopeText = quote.scope && quote.scope.trim().length ? doc.splitTextToSize(quote.scope, sectionTextWidth) : null;
  const notesText = quote.notes && quote.notes.trim().length ? doc.splitTextToSize(quote.notes, sectionTextWidth) : null;

  const drawPagedTextSection = (title, lines, startY, options = {}) => {
    if (!lines || !lines.length) return startY;

    const fillColor = options.fillColor || palette.surface;
    const titleSuffix = options.continuationSuffix || ' (continuação)';
    let cursorY = startY;
    let remainingLines = [...lines];
    let firstPage = true;

    while (remainingLines.length) {
      let sectionTop = cursorY + sectionGapBefore;
      const minBlockHeight = sectionBoxGap + sectionPadTop + sectionPadBottom + sectionLineHeight;
      if (sectionTop + minBlockHeight > pageHeight - bottomMargin) {
        doc.addPage();
        cursorY = margin;
        sectionTop = cursorY + sectionGapBefore;
      }

      const sectionTitle = firstPage ? title : `${title}${titleSuffix}`;
      const boxY = sectionTop + sectionBoxGap;
      const textStartY = boxY + sectionPadTop;
      const maxLines = Math.max(
        1,
        Math.floor((pageHeight - bottomMargin - sectionPadBottom - textStartY) / sectionLineHeight) + 1,
      );
      const linesForPage = remainingLines.slice(0, maxLines);
      remainingLines = remainingLines.slice(linesForPage.length);
      const boxHeight =
        sectionPadTop +
        sectionPadBottom +
        Math.max(1, linesForPage.length) * sectionLineHeight;

      doc.setFontSize(9);
      doc.setTextColor(...palette.muted);
      doc.text(sectionTitle, margin, sectionTop);

      doc.setDrawColor(...palette.border);
      doc.setFillColor(...fillColor);
      doc.roundedRect(margin, boxY, contentWidth, boxHeight, 2.5, 2.5, 'FD');

      doc.setFontSize(sectionFontSize);
      doc.setTextColor(17, 24, 39);
      linesForPage.forEach((line, index) => {
        doc.text(line, margin + sectionPadX, textStartY + index * sectionLineHeight);
      });
  ...
  // 2) Garante apenas que os cards de contato caibam; escopo/notas fluem e quebram pagina depois.
  const contactSectionHeight = clientBoxH + 12;
  if (finalY + contactSectionHeight > pageHeight - bottomMargin) {
    doc.addPage();
    finalY = margin;
  }
  ...
  finalY = clientBoxY + clientBoxH;

  if (scopeText) {
    finalY = drawPagedTextSection('Escopo', scopeText, finalY, {
      fillColor: palette.surface,
    });
  }

  if (notesText) {
    finalY = drawPagedTextSection('Premissas', notesText, finalY, {
  ...
  section('Garantia e suporte', 'Garantia de 60 dias para serviços executados pela Clever Connection. Os equipamentos e materiais fornecidos seguem a garantia padrão do fabricante.');
  section(
